...
{
	initMusic();
    if (g_ViewedPlayer != -1 && g_CivData[g_Players[g_ViewedPlayer].civ].Music) // play specific music if playing as a faction
    {
        global.music.storeTracks(g_CivData[g_Players[g_ViewedPlayer].civ].Music);
        global.music.MatchStartMusic(); // start with the theme of the faction
    }
    else // if spectating, shuffle all the faction music into a single list
    {
        let factions = ["hylian", "gerudo", "goron", "zora", "kokiri", "gohma", "lanayru", "ordona", "moblin", "darknut", "lizalfos", "deku"];
        let musicData = [];
        for (let faction of factions)
            musicData = musicData.concat(g_CivData[faction].Music);
        global.music.ShuffleAllMusic(musicData);
    }
...
function confirmExit()
{
    ...
    let settingsFile = "config/matchsettings.json";
    ...
    let mapSettings = data.attributes; // hyrule campaign uses its own UI screen for victory/defeat
    if (!(mapSettings["mapType"].startsWith("scenario"))) // if this map is a scenario (campaigns)
    {
        messageBox(
            400, 200,
            subject,
...
function updateCinemaPath()
{
	...
	//Engine.GetGUIObjectByName("session").hidden = !g_ShowGUI || isPlayingCinemaPath; // disable for dialogue cutscenes
    Engine.GetGUIObjectByName("minimapPanel").hidden = !g_ShowGUI || isPlayingCinemaPath; // hide the minimap
    if (isPlayingCinemaPath)
        g_Selection.reset(); // no selection should be available during a cutscene (also prevents UI from showing up)
